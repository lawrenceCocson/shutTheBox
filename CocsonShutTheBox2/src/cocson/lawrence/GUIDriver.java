	...
	public void start(Stage stage) throws Exception {
		...
		btnRoll.setOnAction(e -> {
			if (btnRoll.getStyle().equals("")) {
				...
				int tilesDown = 0; // Checks if all the tiles are down
				...
				System.out.println("Tiles up: " + upTiles);
			}
		});
		...
		btnLock.setOnAction(e -> {
			...
			if (btnLock.getStyle().equals("")) {
				...
				for (Button tile : tileBtns) {
					if (tile.getStyle().equals("-fx-background-color: lightgray")) {
						usedCount += 1;
					}
				...
				System.out.println("Used all: " + allUsed);

				if (allUsed) {
					btnRoll.setStyle("-fx-background-color: lightgray");
					btnLock.setStyle("-fx-background-color: lightgray");
					btnEnd.setStyle("-fx-background-color: lightgray");
					message.setText("Perfect Score!");
					for (Button tileBtn : tileBtns) {
